...
export class ProductMakeComponent implements OnInit {
  ...
  onToggleBoxChange(id,index,val, optIndex,wsName) {
    ...
    this.manageMake('submit', index,val);

  }

  // Add, Edit, Cancel Make
  manageMake(action, index,checkedBoxVal='') {
      ...
      case 'submit':
        if(this.submitActionFlag) {
          ...
          setTimeout(() => {
            ...
            this.makeAction(index, apiData,checkedBoxVal);       
          }, 100);
        }
        break;  
    }
  ...
  // Make Submit Action
  makeAction(index, makeApiData,checkedBoxVal) {
    let makeIndex = 0;
