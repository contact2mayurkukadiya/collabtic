...
export class DomainUrlComponent implements OnInit {
 ...
 @ViewChild('myInput1') myInput1Element: ElementRef;
  ...
  checkEmail(){    
    ...
    domainData.append('api_key', Constant.ApiKey);
    ...
    domainData.append('step', '1');

    this.authenticationService.login(domainData).subscribe(
      response => { 
        this.loading1 = false;
        if(response.status == "Success") {         
          this.domainDataVal = response.data;
