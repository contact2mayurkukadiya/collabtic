...
export class LandingLeftSideMenuComponent implements OnInit, AfterViewInit {
  ...
  async ngOnInit() {
    ...
    this.chatService.channel.onmessage = (event) => {
      ...
      if (this.chatService.workstreamType == event.data.type) {
        ...
        for (var iar in this.chatService.workstreamArr) {
    ...
    if (localStorage.getItem(LocalStorageItem.reloadChatGroupId) == null || localStorage.getItem(LocalStorageItem.reloadChatGroupId) == undefined || localStorage.getItem(LocalStorageItem.reloadChatGroupId) == '') {
      this.defaultWorkstream = localStorage.getItem('landing-page-workstream');
    ...
    if (this.responseResults) {
      ...
      if (response.status == "Success") {
        ...
        for (var m in this.menuListloaded) {
        ...
        for (var e in this.accorodianMenu) {
    ...
    this.subscription = this.notification.visibility.pipe(debounceTime(500)).subscribe(async res => {
      if (res) {
  ...
  getWorkstreamwithCountLists(emitValue, wsId: any = '') {
      ...
      if (res.status = "Success") {
        ...
        for (var ws in resData) {
          ...
          if (this.chatService.workstreamArr.length > 0) {
            ...
            for (var wsd in this.chatService.workstreamArr) {
              ...
              */
                ...
                this.chatService.workstreamArr.push({ wsId: ws_id, wsName: ws_name, cConfig: chat_conf, wsImg: ws_img, wsCount: resData[ws].totalCount, wsNew: ws_isNew, contentType: ws_contentType, wsEditAccess: ws_editAccess, wsAction: resData[ws].workstreamAction, newthreadpush: false, removeCount: false });
              }
            }
          }
          else {
        ...
        if (this.currentWorkstreamIdInfo != Number(this.notificationData?.chatGroupId)) {
          let totalNewWorkstreamMessage = 0;
          for (var i = 0; i < this.chatService.workstreamArr.length; i++) {
    ...
    for (var ws in this.chatService.workstreamArr) {
  ...
  LoadGroupChat(group: any) {
    ...
    for (var iar in this.chatService.grstreamArr) {
  ...
  LoadDirectChat(group: any) {
    ...
    // console.log(pushitem)
    for (var iar in this.chatService.dmstreamArr) {
      ...
      this.commonService.GetGroupAndDirectMessagewithCount(apiFormData).subscribe((response) => {
        ...
        if (res.status = "Success") {
          ...
          if (this.chatService.grstreamArr) {
            for (var i = 0; i < this.chatService.grstreamArr.length; i++) {
          ...
          if (this.chatService.dmstreamArr) {
            for (var i = 0; i < this.chatService.dmstreamArr.length; i++) {
  ...
  openDialogNotification(wid) {
    ...
    for (var cw in this.chatService.workstreamArr) {
    ...
    for (var cs in this.currentSettingArr) {
