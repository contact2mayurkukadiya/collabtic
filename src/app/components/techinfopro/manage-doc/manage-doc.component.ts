...
export class ManageDocComponent implements OnInit, OnDestroy {
      ...
      for(let sec of sections) {
        ...
        for(let f of fieldLists) {
          ...
          if(f.cellArray.length > 0) {
            ...
            if(this.docId > 0 || (this.docId == 0 && f.fieldName == 'workstreams' && f.autoselection == 1)) {
              f.recentShow = false;
              let threadVal;
              if(this.docId > 0) {
              ...
              if(f.selection == 'multiple') {
                ...
                if(f.fieldName == 'uploadContents') {
                  ...
                  if(this.pageInfo.attachmentItems.length>0){  
                    for (var att in this.pageInfo.attachmentItems) { 
  ...
  docSubmit() {
    ...
    if(this.docId > 0) {
      ...
      if(this.docType == 1) {
          ...
          this.commonApi.createDocument(docFormData).subscribe((response) => {
            ...
            if(response.status == "Success") {
              ...
              if(Object.keys(this.uploadedItems).length > 0 && this.uploadedItems.items.length > 0 && uploadCount > 0) {
                msgFlag = false;
                this.pageInfo.bulkUpload = false;               
                this.docUpload(postId, docId, pushFormData);
              }
              else{
        ...
        localStorage.setItem('documentTab', 'uploaded');
        this.bulkDocSubmit(docFormData, pushFormData, modalRef);        
      }
    }
  }

  // Bulk Doc Submit
  async bulkDocSubmit(docFormData, pushFormData, modalRef) {
    ...
    for(let u in uploadedItems) {
      ...
      await this.bulkDocCreation(docFormData, pushFormData, modalRef, uploadedItems[u], u, itemLen, uploadCount);
    }
  }

  // Bulk Doc Creation
  bulkDocCreation(docFormData, pushFormData, modalRef, item, index, itemLen, uploadCount) {
  ...
  // Document Upload
  docUpload(docId, postId, pushFormData) {
    this.pageInfo.threadUpload = false;
    this.pageInfo.contentType = this.contentType;
    this.pageInfo.dataId = postId;
