...
export class TvsssoComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void { 
    ...
    this.addForm3 = this.formBuilder.group({
      email: ['', [Validators.required, Validators.email,Validators.pattern('^[A-Za-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,8}$')]]                       
    });
  ...
  addUserType() {  
    ...
    modalRef.componentInstance.filteredUsers.subscribe((receivedService) => {
      ...
      if(!receivedService.empty)  {       
        ...
        console.log(userTypeData.uTypeId);
        console.log(userTypeData.uTypeName);
    ...
    if(this.enableCols){
      ...
      switch(type){
        ...
        case 'branch':
          ...
          console.log(this.userBranchListData);        
          this.serverErrorMsg = '';
          this.serverError = false;  
          let innerHeight = window.innerHeight;
          let userBranchList = {
        ...
        case 'language':
          ...
          console.log(this.userLanguageListData);        
          this.userBranchInputFlag = true;
          this.serverErrorMsg = '';
          this.serverError = false;  
