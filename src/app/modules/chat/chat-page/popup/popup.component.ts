...
export class PopupComponent implements OnInit {
  ...
  prepareAllDomainUserList(isSelected): DomainUserChat {
    ...
    domainUserChat.selectedUsers = isSelected;
  ...
  prepareMemberData(chatType): MemberToGroup {
    ...
    for (var i = 0; i < this.newMeberList.length; i++) {
      memberUserId.push(this.newMeberList[i].userId);
    }
    if (this.isNewGroup) {
      memberUserId.push(this.userId);
    }
    for (var j = 0; j < this.removedMeberList.length; j++) {
  ...
  ClearAllMember() {
    for (var i = 0; i < this.memberSelectedList.length; i++) {
  ...
  preparePushNotification(
    ...
    for (var j = 0; j < removememberlist.length; j++) {
